...
export default function Header() {
  ...
  async function reverseGeocode(latitude, longitude) {
    try {
      ...
      console.error("Ocorreu um erro ao chamar a API ou processar os dados:", error);
  ...
  async function success(position) {
    ...
    const localResult = await reverseGeocode(position.coords.latitude, position.coords.longitude);
  ...
  function mostrarErro(error) {
    ...
    switch (error.code) {
      ...
      case error.POSITION_UNAVAILABLE:
        ...
        break;
      case error.TIMEOUT:
        mensagem = "Pedido de localização expirou.";
        break;
      default:
  ...
  function getLocation() {
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(success, mostrarErro);
    } else {
  ...
  useEffect(() => {
    console.log("Componente Header montado. Iniciando busca por localização...");
    getLocation();
  }, []); 

  return (
    <header className="flex items-center justify-center relative h-20 md:fixed md:top-0 md:left-0 md:right-0 md:bg-[#4C0000] md:justify-between md:px-6 md:gap-x-4 md:h-25 xl:h-30">
      ...
      <div className="hidden md:block grow max-w-3/5">
